...
pub struct SshCommand {
    ...
    /// Reload the kexec installer even when the target already appears to be in one.
    #[arg(long)]
    force_kexec: bool,
...
pub fn build_plan(command: &SshCommand) -> Result<DeploymentPlan, AppError> {
    ...
    Ok(DeploymentPlan {
        ...
        force_kexec: command.force_kexec,
...
pub fn render_script(plan: &DeploymentPlan) -> String {
    ...
    script.push_str(&format!(
        "NIXOS_KEXEC_FORCE_KEXEC={}; export NIXOS_KEXEC_FORCE_KEXEC\n\n",
        if plan.force_kexec { 1 } else { 0 }
    ));
...
fn print_plan(output: &mut impl Write, plan: &DeploymentPlan) -> Result<(), AppError> {
    ...
    writeln!(output, "force kexec: {}", plan.force_kexec)?;
...
fn detect_remote_installer_command(command: &SshCommand) -> PlanCommand {
    ...
    local_command(
        ...
        raw_script_argv(format!(
            "NIXOS_KEXEC_REMOTE_IS_INSTALLER=0; if {probe}; then NIXOS_KEXEC_REMOTE_IS_INSTALLER=1; if [ \"${{NIXOS_KEXEC_FORCE_KEXEC:-0}}\" = 1 ]; then echo 'nixos-kexec: target is already in the kexec installer; force-kexec requested' >&2; else echo 'nixos-kexec: target is already in the kexec installer; skipping kexec handoff' >&2; fi; fi; export NIXOS_KEXEC_REMOTE_IS_INSTALLER"
        )),
    )
}
...
fn skip_when_already_installer(command: PlanCommand) -> PlanCommand {
    ...
    local_command(
        ...
        mutates,
        raw_script_argv(format!(
            "if [ \"${{NIXOS_KEXEC_REMOTE_IS_INSTALLER:-0}}\" = 1 ] && [ \"${{NIXOS_KEXEC_FORCE_KEXEC:-0}}\" != 1 ]; then echo 'nixos-kexec: already in installer; skipping {description}' >&2; else {rendered}; fi"
        )),
    )
}
...
mod tests {
    ...
    fn rendered_script_keeps_commands_reviewable() {
        ...
        assert!(script.contains("NIXOS_KEXEC_FORCE_KEXEC=0; export NIXOS_KEXEC_FORCE_KEXEC"));
    ...
    #[test]
    fn force_kexec_reloads_even_when_target_is_already_installer() {
        let temp = tempfile::tempdir().unwrap();
        let mut command = fixture_command(&temp);
        command.force_kexec = true;

        let plan = build_plan(&command).unwrap();
        let script = render_script(&plan);

        assert!(plan.force_kexec);
        assert!(script.contains("NIXOS_KEXEC_FORCE_KEXEC=1; export NIXOS_KEXEC_FORCE_KEXEC"));
        assert!(script.contains("force-kexec requested"));
        assert!(script.contains("[ \"${NIXOS_KEXEC_FORCE_KEXEC:-0}\" != 1 ]"));
    }
